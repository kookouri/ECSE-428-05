...
Scenario Outline: User successfully leaves a review
  ...
  Then a new review for "<item_name>" with rating "<rating>", comment "<comment>" and date "<datePosted>" shall be created
...
Scenario Outline: User fails to leave a review due to invalid input
  When the user with email "<email>", phone "<phone>", and password "<password>" attempts to submit a review for "<item_name>" with a rating of "<rating>", comment "<comment>" and date "<datePosted>"
  Then an error message "<error_message>" shall be displayed
  And the total number of reviews for "<item_name>" shall be "0"
...
Scenario: Unauthenticated user tries to leave a review
  ...
  And the total number of reviews for "McGill Hoodie" shall be "0"
