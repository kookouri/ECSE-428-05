Feature: Checking Out The Shopping Cart (ID011)
    ...
    Background:
        ...
        And the following items exist in the system (ID011)
            ...
            | 1  | Desautels Pencil Case   | 12.0    | Pencil case with Desautels brand | School Supply | 
    ...
    Scenario Outline: Successfully check out a non-empty shopping cart.
        ...
        Then the message "Successfully checked-out the shopping cart" will be raised.
    ...
    Scenario Outline: Unsuccessfully check out an empty shopping cart.
        ...
        When I attempt to check-out the shopping cart
        Then the error "Empty shopping cart: Item amount must be larger than 0" shall be raised (ID009)
        And the number of items in my cart should be "0" (ID011)
