Feature: Change User Information (ID004)
  ...
  Scenario: Successfully update user name and phone number
    When the user updates the "name" field to "Johnny Doe" and the "phone number" field to "321-654-9870" for user with id "01"
    Then the user profile should show the updated "name" as "Johnny Doe"
    And the "phone number" should display "321-654-9870"
  ...
  Scenario: Successfully update user email
    ...
    And the "phone number" should remain "123-456-4890"

  Scenario: Successfully update user password
    When the user changes the password for user with id "02" to "newPassword2"
